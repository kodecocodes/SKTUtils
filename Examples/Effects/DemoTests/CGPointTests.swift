...
class CGPointTests: XCTestCase {
  ...
  func testInitWith45DegreeAngle() {
    let a = π/4.0
    let pt = CGPoint(angle: a)
    XCTAssertEqual(pt.x, 1.0/sqrtf(2.0))
    XCTAssertEqual(pt.y, 1.0/sqrtf(2.0))
  }

  func testInitWith90DegreeAngle() {
    let a = π/2.0
    let pt = CGPoint(angle: a)
    XCTAssertEqualWithAccuracy(pt.x, 0.0, FLT_EPSILON)
  ...
  func testInitWith180DegreeAngle() {
    ...
    let pt = CGPoint(angle: a)
    XCTAssertEqual(pt.x, -1.0)
    XCTAssertEqualWithAccuracy(pt.y, 0.0, FLT_EPSILON)
  }
  ...
  func test45DegreeAngle() {
    let pt = CGPoint(x: 1.0/sqrtf(2.0), y: 1.0/sqrtf(2.0))
  ...
  func testMinus135DegreeAngle() {
    let pt = CGPoint(x: -1.0/sqrtf(2.0), y: -1.0/sqrtf(2.0))
    XCTAssertEqualWithAccuracy(pt.angle, -3.0*π/4.0, FLT_EPSILON)
  }
  ...
  func testLength() {
    ...
    XCTAssertEqual(pt.length(), sqrtf(2.0))
  }
  ...
  func testDistance() {
    XCTAssertEqualWithAccuracy(pt1.distanceTo(pt2), 100.6230621337, FLT_EPSILON)
  }

  func testThatLengthEqualsDistance() {
    XCTAssertEqualWithAccuracy(pt1.distanceTo(pt2), (pt1 - pt2).length(), FLT_EPSILON)
  }

  func testNormalized() {
    let normalized = pt1.normalized()
    XCTAssertEqualWithAccuracy(normalized.x, 2.0/sqrtf(5.0), FLT_EPSILON)
    XCTAssertEqualWithAccuracy(normalized.y, 1.0/sqrtf(5.0), FLT_EPSILON)
  }
  ...
  func testThatNormalizeReturnsNewValue() {
    pt1.normalize()
    XCTAssertEqualWithAccuracy(pt1.x, 2.0/sqrtf(5.0), FLT_EPSILON)
    XCTAssertEqualWithAccuracy(pt1.y, 1.0/sqrtf(5.0), FLT_EPSILON)
  }
  ...
  func testLerp() {
    ...
    for var t = 0.0; t <= 1.0; t += 0.1, ++i {
      let lerped = lerp(start: start, end: end, t: Float(t))
